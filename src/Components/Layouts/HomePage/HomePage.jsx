...
import { GetProductApi } from "../../Component/ProductData.js";
	...
	return (
		<div>
			...
			<div className="search-bar-container">
				<Form onSubmit={submitHandler} className="input-wrapper d-flex flex-column p-0">
					<InputGroup className="input-wrapper w-100 align-items-center">
						<FaSearch className="search-icon" />
						<Form.Control onChange={(e) => setSearch(e.target.value)} placeholder="Search products" className="border-0" id="moreInputStyle" />
					</InputGroup>
				</Form>
				...
				<div className="d-flex flex-wrap justify-content-center gap-3 p-5 mt-4 mb-5 w-100" id="Results">
					{data.filter((item) => {
						...
						.map((item, index) => (<ProductCardHome key={index} data={item} />))}
				</div>
			</div>
		</div>
	);
