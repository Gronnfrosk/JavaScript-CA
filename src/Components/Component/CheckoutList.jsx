...
export default function ChartList(props) {
	...
	if (cartItems.length !== 0) {
		const itemsPriceSave = cartItems.reduce((a, b) => a + b.qty * (b.price - b.discountedPrice), 0);
			...
			<tbody>
				{cartItems.map((item, index) => (
					<tr key={index}>
						...
						<td >{item.title}</td>
						<td className="text-center"><Button variant="light border border-dark" onClick={() => onAdd(item)} >+</Button><Button variant="dark fw-bold" onClick={() => onRemove(item)}>-</Button></td>
